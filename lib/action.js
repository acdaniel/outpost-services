...
Action.prototype.finalizeEntity = function (entity, ctx) {
  ...
  if (entity instanceof Collection) {
    ...
    for (var i = 0, l = entity.body.items.length; i < l; i++) {
      ...
      this.finalizeEntity(entity.body.items[i], itemCtx);
    }
  }
  ...
  var embed = ctx.params && ctx.params._embed ? ctx.params._embed.toLowerCase().split(/[\s,]+/) : false;
  var promises = [];
  for (var r in rels) {
    var rel = rels[r];
    if (entity instanceof Collection) {
    ...
    if (rel.oneToMany) {
      promises.push(_populateOneToManyRel(entity, rel, relCtx, isEmbedded, paramsSource));
    } else {
      promises.push(_populateOneToOneRel(entity, rel, relCtx, isEmbedded, paramsSource));
    }
  }
...
function _populateOneToOneRel (entity, rel, ctx, embed, obj) {
  for (var rp in rel.params) {
    var relParam = rel.params[rp];
    if (relParam.substr(0, 1) === '$') {
      if (relParam.substr(-2) === '++') {
        ctx.params[rp] = parseInt(obj[relParam.substr(1, relParam.length - 3)]) + 1;
      } else if (relParam.substr(-2) === '--') {
        ctx.params[rp] = parseInt(obj[relParam.substr(1, relParam.length - 3)]) - 1;
      } else {
        ctx.params[rp] = obj[relParam.substr(1)];
      }
    } else {
      ctx.params[rp] = relParam;
    }
  }
  if (embed) {
    return rel.getTargetEntity(ctx)
    ...
    entity.link(rel.name, rel.buildTargetLink(ctx.params));
    return true;
  }
}

function _populateOneToManyRel (entity, rel, ctx, embed, obj) {
  var key = null;
  var many = [];
  var promises = [];
  for (var rp in rel.params) {
  ...
  if (!many) { return; }
  if (embed) {
    entity.embedded[rel.name] = [];
  } else {
    entity.links[rel.name] = [];
  }
  for (var i = 0, l = many.length; i < l; i++) {
    ctx.params[key] = many[i];
    if (embed) {
      promises.push(
        rel.getTargetEntity(ctx)
          .then(function (childEntity) {
            entity.embed(rel.name, childEntity);
            return entity;
          })
