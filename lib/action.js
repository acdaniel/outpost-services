...
Action.prototype.checkAuthorization = function (granted) {
  ...
  var commonScope = _.intersection(this.auth, granted);
...
Action.prototype.invoke = function (ctx) {
  ...
  if (!this.checkAuthorization(granted)) {
    return q.reject(new errors.Unauthorized());
  }
  try {
    if (this.input) {
      this.input.validate(ctx.input || {});
    }
    return q(di.invoke(this.fn, { ctx: ctx, params: ctx.params, input: ctx.input, auth: ctx.auth }, this))
...
Action.prototype.finalizeEntity = function (entity, ctx) {
  ...
  // populate rels
  var paramsSource = _.merge({}, entity.body, ctx.params.uri, ctx.params.input);
  if (!resource.getRel('self')) {
    entity.link('self', resource.buildLink(paramsSource, 'self'));
  }
  var rels = resource.getRels();
  var embed = ctx.input && ctx.input._embed ? ctx.input._embed.split(/,?\s+/) : false;
  var promises = [];
  for (var r in rels) {
    var rel = rels[r];
    if (entity instanceof Collection) {
    ...
    if (isEmbedded) {
      promises.push(rel.getTargetEntity(relCtx)
